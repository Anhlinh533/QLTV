...
BEGIN
	...
	FROM INSERTED I, PHIEUNHAPSACH
	WHERE I.IDPhieuNhap = IDPhieuNhap
	SELECT @NAMXB = NamXB
	FROM INSERTED I, SACH
	WHERE I.IDSach = IDSach
...
BEGIN
	...
	IF((@NGAYHETHAN <= @NGAYMUON) OR (@TINHTRANG = N'Cho mượn') OR EXISTS (SELECT * FROM PHIEUMUON A 
																		   WHERE HanTra < @NGAYMUON 
																		   AND NOT EXISTS(SELECT * FROM CT_PHIEUTRA B WHERE A.IDPhieuMuon = B.IDPhieuMuon)))
	BEGIN
